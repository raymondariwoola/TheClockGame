...
| 11 | PWA install, update, and offline-status polish | M-H | M | None | **Selected; PWA-1 and PWA-2 complete locally.** Installability and run-safe updates are implemented; connection status remains. |
...
| [x] | PWA update / offline polish | **In progress: install and safe-update phases complete; offline/cloud status pending** | FE-1 |
